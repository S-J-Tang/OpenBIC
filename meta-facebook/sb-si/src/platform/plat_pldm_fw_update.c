...
pldm_fw_update_info_t PLDMUPDATE_FW_CONFIG_TABLE[] = {
	...
	},
	{
		.enable = true,
		.comp_classification = COMP_CLASS_TYPE_DOWNSTREAM,
		.comp_identifier = SI_COMPNT_P0V895_PEX,
		.comp_classification_index = 0x00,
...
static bool get_pcie_switch_fw_version(void *info_p, uint8_t *buf, uint8_t *len)
{
	...
	*len = bin2hex((uint8_t *)&version, 4, buf, 8);
	...
	LOG_INF("PEX90144 %s SBR version: 0x%08x", sensor_name, version);
						 ...
						 char *sensor_name)
{
	...
	for (uint8_t i = 0; i < ARRAY_SIZE(si_vr_compnt_mapping_sensor_table); i++) {
