...
#define U618_WORKAROUND_PWR_EN_BIT 0
...
bool check_312_5MHz_init_status(void)
{
	...
	pwr_en = !!(pwr_en & BIT(U618_WORKAROUND_PWR_EN_BIT));
	...
	/* Follow the Rainbow flow: only re-initialize U618 while PWR_EN is low. */
	if (pwr_en) {
		LOG_ERR("Power is on with unexpected CLK U618 init status");
		error_log_event(CLK_312_5MHZ_REINIT_ERR_CODE, LOG_ASSERT);
		return false;
	}

	LOG_WRN("PWR_EN=%d, CLK U618 registers require workaround: "
		"0x00A8=%02x%02x%02x%02x, 0x0080=%02x, 0x0088=%02x%02x; applying workaround",
		pwr_en, read_00a8[0], read_00a8[1], read_00a8[2], read_00a8[3], read_0080[0],
	...
	/* Retain the incident if PWR_EN becomes asserted during re-init. */
	if (has_error || pwr_en) {
