...
uint16_t get_power_capping_avg_power(uint8_t vr_idx, uint8_t lv)
{
	...
	case CAPPING_LV_IDX_LV2:
		if (vr_idx == CAPPING_VR_IDX_NUWA0) {
			tmp_idx = ADC_IDX_NUWA0_1;
		} else if (vr_idx == CAPPING_VR_IDX_NUWA1) {
			tmp_idx = ADC_IDX_NUWA1_1;
		}
		break;
	case CAPPING_LV_IDX_LV3:
		if (vr_idx == CAPPING_VR_IDX_NUWA0) {
			tmp_idx = ADC_IDX_NUWA0_2;
		} else if (vr_idx == CAPPING_VR_IDX_NUWA1) {
			tmp_idx = ADC_IDX_NUWA1_2;
		}
		break;
	default:
		break;
...
void set_power_capping_time_w(uint8_t vr_idx, uint8_t lv, uint16_t value)
{
	...
	case CAPPING_LV_IDX_LV2:
		if (vr_idx == CAPPING_VR_IDX_NUWA0) {
			adc_set_averge_times(ADC_IDX_NUWA0_1, value);
		} else if (vr_idx == CAPPING_VR_IDX_NUWA1) {
			adc_set_averge_times(ADC_IDX_NUWA1_1, value);
		}
		break;
	case CAPPING_LV_IDX_LV3:
		if (vr_idx == CAPPING_VR_IDX_NUWA0) {
			adc_set_averge_times(ADC_IDX_NUWA0_2, value);
		} else if (vr_idx == CAPPING_VR_IDX_NUWA1) {
			adc_set_averge_times(ADC_IDX_NUWA1_2, value);
		}
		break;
	default:
		break;
	...
	if (lv == CAPPING_LV_IDX_LV1) {
		power_capping_info.time_w[CAPPING_VR_IDX_NUWA0][lv] = value;
		power_capping_info.time_w[CAPPING_VR_IDX_NUWA1][lv] = value;
	} else {
...
void set_power_capping_threshold(uint8_t vr_idx, uint8_t lv, uint16_t value)
{
	...
	case CAPPING_LV_IDX_LV1:
		if (vr_idx == CAPPING_VR_IDX_NUWA0) {
			if (!set_power_capping_vr_oc_warn_limit(ADC_IDX_NUWA0_1, value)) {
				return;
			}
		} else if (vr_idx == CAPPING_VR_IDX_NUWA1) {
			if (!set_power_capping_vr_oc_warn_limit(ADC_IDX_NUWA1_1, value)) {
				return;
			}
		}
		break;
	case CAPPING_LV_IDX_LV2:
		if (vr_idx == CAPPING_VR_IDX_NUWA0) {
			set_adc_ucr(ADC_IDX_NUWA0_1, value);
		} else if (vr_idx == CAPPING_VR_IDX_NUWA1) {
			set_adc_ucr(ADC_IDX_NUWA1_1, value);
		}
		break;
	case CAPPING_LV_IDX_LV3:
		if (vr_idx == CAPPING_VR_IDX_NUWA0) {
			set_adc_ucr(ADC_IDX_NUWA0_2, value);
		} else if (vr_idx == CAPPING_VR_IDX_NUWA1) {
			set_adc_ucr(ADC_IDX_NUWA1_2, value);
		}
		break;
	default:
		break;
...
void plat_power_capping_init()
{
	...
	if (plat_read_cpld(CPLD_OFFSET_POWER_CAPPING_LV1_TIME, &data, 1)) {
		...
		if (data >= 10 && data <= 100) {
			power_capping_info.time_w[CAPPING_VR_IDX_NUWA0][CAPPING_LV_IDX_LV1] = data;
			power_capping_info.time_w[CAPPING_VR_IDX_NUWA1][CAPPING_LV_IDX_LV1] = data;
			LOG_INF("cpld lv1 time window: %d, nuwa0: %d us, nuwa1: %d us", data,
