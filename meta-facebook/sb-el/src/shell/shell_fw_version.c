...
static const asic_item_t asic_list[BOOT0_MAX] = {
	...
	{ BOOT0_NUWA0, "NUWA0", I2C_BUS12, 0x32 },
	{ BOOT0_NUWA1, "NUWA1", I2C_BUS12, 0x32 },
};
...
void cmd_get_fw_version_vr(const struct shell *shell, size_t argc, char **argv)
{
	...
	for (int i = COMPNT_VR_1; i <= COMPNT_VR_3V3; i++) {
		...
		if (i == COMPNT_HAMSA || i == COMPNT_NUWA0 || i == COMPNT_NUWA1)
			continue;
		...
		if (cfg->type == sensor_dev_mp2891 || cfg->type == sensor_dev_mp29816a)
			shell_print(shell, "%-8x|%-40s|    %04x|%04x", i, sensor_name, version,
			 ...
			 cfg->type == sensor_dev_raa228249 || cfg->type == sensor_dev_mp2971 || cfg->type == sensor_dev_mp29526)
...
void cmd_get_fw_version_asic_from_flash(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (plat_get_image_crc_checksum_from_flash(COMPNT_NUWA0, &version, &crc32)) {
		shell_print(shell, "NUWA0 boot0 VER : %08x | CRC32 : %08x", version, crc32);
	} else {
		shell_warn(shell, "NUWA0 boot0 VER and CRC reading failed");
	}

	if (plat_get_image_crc_checksum_from_flash(COMPNT_NUWA1, &version, &crc32)) {
		shell_print(shell, "NUWA1 boot0 VER : %08x | CRC32 : %08x", version, crc32);
	} else {
		shell_warn(shell, "NUWA1 boot0 VER and CRC reading failed");
	}
