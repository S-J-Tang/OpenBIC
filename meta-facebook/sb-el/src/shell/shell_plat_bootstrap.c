...
static int bootstrap_set_dbb_cmd(const struct shell *shell, bool is_perm)
{
	if (!bootstrap_set_dbb(is_perm)) {
		shell_error(shell, "Failed to set DBB bootstrap preset");
		return -1;
	}
...
static int cmd_bootstrap_set(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (!strcmp(argv[1], "DBB")) {
		...
		return bootstrap_set_dbb_cmd(shell, argc == 3);
	}
