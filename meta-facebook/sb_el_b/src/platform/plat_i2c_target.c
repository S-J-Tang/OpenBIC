...
voltage_rail_mapping_sensor voltage_rail_mapping_table[] = {
	...
	{ CONTROL_VOL_VR_ASIC_P0V85_NUWA0_VDD_REG, VR_RAIL_E_ASIC_P0V85_NUWA0_VDD },
	{ CONTROL_VOL_VR_ASIC_P0V85_NUWA1_VDD_REG, VR_RAIL_E_ASIC_P0V85_NUWA1_VDD },
};
...
uint8_t vr_pwr_sensor_table[] = {
	SENSOR_NUM_ASIC_P0V85_NUWA0_VDD_PWR_W,
...
void vr_power_reading(uint8_t *buffer, size_t buf_size)
{
	...
	[6:7] - P0V85_NUWA0_VDD (Unit: W)
	[8:9] - P0V85_NUWA1_VDD (Unit: W)
	...
	*/
	...
	for (size_t i = 0; i < ARRAY_SIZE(vr_pwr_sensor_table); i++) {
		...
		if (vr_pwr_sensor_table[i] != SENSOR_NUM_ASIC_P0V85_NUWA0_VDD_PWR_W &&
		    vr_pwr_sensor_table[i] != SENSOR_NUM_ASIC_P0V85_NUWA1_VDD_PWR_W) {
			x += milivolt;
		}
	}
	int reading;
...
static bool command_reply_data_handle(void *arg)
{
	...
	if (data->wr_buffer_idx >= 1) {
		if (data->wr_buffer_idx == 1) {
			...
			case CONTROL_VOL_VR_ASIC_P0V85_NUWA0_VDD_REG:
			case CONTROL_VOL_VR_ASIC_P0V85_NUWA1_VDD_REG: {
...
uint8_t vr_pwr_alert_table[] = {
	SENSOR_NUM_ASIC_P0V85_NUWA0_VDD_PWR_W,	  SENSOR_NUM_ASIC_P0V85_NUWA1_VDD_PWR_W,
