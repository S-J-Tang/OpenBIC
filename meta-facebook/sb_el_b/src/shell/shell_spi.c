...
void cmd_spi_enable_nuwa0(const struct shell *shell, size_t argc, char **argv)
{
	...
	gpio_set(QSPI_CPLD_SEL_1, 0);
	if (!set_cpld_bit(RESET, NUWA0_POWER_ON_RESET_PLD_L_BIT, 0) ||
	    !set_cpld_bit(RESET, NUWA0_SYS_RST_PLD_L_BIT, 0)) {
...
void cmd_spi_enable_nuwa1(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (!set_cpld_bit(RESET, NUWA1_POWER_ON_RESET_PLD_L_BIT, 0) ||
	    !set_cpld_bit(RESET, NUWA1_SYS_RST_PLD_L_BIT, 0)) {
