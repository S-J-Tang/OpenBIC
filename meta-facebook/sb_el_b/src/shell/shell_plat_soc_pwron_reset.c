...
#define ARKE_POWER_RESET_OFFSET 0x00
...
typedef enum {
	...
	NUWA0_POWER_ON_RESET_L_ID,
	NUWA1_POWER_ON_RESET_L_ID,
	HAMSA_SYS_RST_L_ID,
	NUWA0_SYS_RST_L_ID,
	NUWA1_SYS_RST_L_ID,
...
static const soc_pwron_reset_item_t soc_pwron_reset_list[SOC_PWRON_RESET_MAX_ID] = {
	...
	{ NUWA0_POWER_ON_RESET_L_ID, "NUWA0_POWER_ON_RESET_L", NUWA0_POWER_ON_RESET_BIT },
	{ NUWA1_POWER_ON_RESET_L_ID, "NUWA1_POWER_ON_RESET_L", NUWA1_POWER_ON_RESET_BIT },
	{ HAMSA_SYS_RST_L_ID, "HAMSA_SYS_RST_L", HAMSA_SYS_RST_BIT },
	{ NUWA0_SYS_RST_L_ID, "NUWA0_SYS_RST_L", NUWA0_SYS_RST_BIT },
	{ NUWA1_SYS_RST_L_ID, "NUWA1_SYS_RST_L", NUWA1_SYS_RST_BIT },
};
...
static int cmd_soc_pwron_reset_override(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (!strcmp(argv[0], "override")) {
		if (!strcmp(argv[1], "1")) {
			data = ARKE_BOARD_POWER_ENABLE;
			if (!plat_write_cpld(ARKE_POWER_RESET_OFFSET, &data)) {
				shell_error(shell, "plat soc_pwron_reset set failed");
				return -1;
			}
		} else if (!strcmp(argv[1], "0")) {
			data = ARKE_BOARD_POWER_DISABLE;
			if (!plat_write_cpld(ARKE_POWER_RESET_OFFSET, &data)) {
				shell_error(shell, "plat soc_pwron_reset set failed");
				return -1;
			}
...
static int cmd_soc_pwron_reset_get(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (!plat_read_cpld(ARKE_POWER_RESET_OFFSET, &reg_val, 1)) {
		LOG_DBG("plat_read_cpld failed: offset=0x%02x", ARKE_POWER_RESET_OFFSET);
...
static int cmd_soc_pwron_reset_set(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (!plat_write_cpld(ARKE_POWER_RESET_OFFSET, &reg_val)) {
		LOG_DBG("plat_write_cpld failed: offset=0x%02x val=0x%02x", ARKE_POWER_RESET_OFFSET,
			reg_val);
