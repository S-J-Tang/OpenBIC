...
{
	{ NC_SPI_NUWA0_CRM_MUX, NC_SPI_NUWA0_CRM_MUX_IN1, "NC_SPI_NUWA0_CRM_MUX_IN1" },
	{ NC_SPI_NUWA1_CRM_MUX, NC_SPI_NUWA1_CRM_MUX_IN1, "NC_SPI_NUWA1_CRM_MUX_IN1" },
...
static int cmd_bpsi_set(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (!strcmp(name, "NC_SPI_NUWA0_CRM_MUX_IN1"))
	{
		if (gpio_set(NC_SPI_NUWA0_CRM_MUX_IN1, set_val) == -1)
		{
			shell_print(shell, "set NC_SPI_NUWA0_CRM_MUX_IN1 fail!");	
			return 0;
		}
			
	}
	else if (!strcmp(name, "NC_SPI_NUWA1_CRM_MUX_IN1"))
	{
		if (gpio_set(NC_SPI_NUWA1_CRM_MUX_IN1, set_val) == -1)
		{
			shell_print(shell, "set NC_SPI_NUWA1_CRM_MUX_IN1 fail!");
			return 0;
		}
