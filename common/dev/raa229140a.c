...
/*
 * TODO(you): CONFIG ID TABLE INDEX
 *
 * On the raa229621 the Configuration ID is carried on data line #7. For the
 * RAA229140A the configuration table extends to 24 lines, so the config-ID line
 * index moves. Set VR_RAA_CFG_ID to the correct 0-based data-line index (the
 * value shown here (24) is a PLACEHOLDER - confirm it against the actual hex
 * image / programming guide for the 229140A).
 *
 * NOTE: The CRC lines are counted from the END of the file, so if the number of
 * config lines changed you must ALSO re-check VR_RAA_GEN3_FILE_HEAD and the two
 * *_CRC absolute line numbers below.
 */
#define VR_RAA_CFG_ID (24) // TODO(you): confirm the config-ID data-line index

#define VR_RAA_GEN3_FILE_HEAD (5)
/*
 * TODO(you): CRC LINE OFFSETS
 * These are the absolute line numbers of the legacy / production CRC lines in
 * the hex file, minus the file header. Verify both numbers for the 229140A
 * image - they change whenever the config table length changes.
 */
#define VR_RAA_GEN3_LEGACY_CRC (276 - VR_RAA_GEN3_FILE_HEAD) // TODO(you): confirm
#define VR_RAA_GEN3_PRODUCTION_CRC (290 - VR_RAA_GEN3_FILE_HEAD) // TODO(you): confirm
