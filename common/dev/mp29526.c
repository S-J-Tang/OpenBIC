...
/*
 * STORE_USER writes pages 0/1/2 plus two CRC sets into NVM. MPS AN255 (the
 * MP29426-family configuration user guide, closest documented reference for
 * this part's ATE config file format) specifies a 1s wait after STORE_USER
 * and a 100ms wait after RESTORE_USER before checking STATUS_CML.
 */
 ...
 * doesn't fit this platform's heap (CONFIG_HEAP_MEM_POOL_SIZE), so this
 ...
 *
 * Column layout and multi-config programming sequence follow MPS AN255
 * ("Configuration User Guide for the MP29426 family") - the closest
 * documented reference for this ATE config file format; MP29526 is not
 * listed by name in that app note, so anything below sourced from it is a
 * best effort pending vendor confirmation:
 *
 ...
 *         digit field is a plain page number with no group - see
 *         mp29526_parse_group_page())
 *   col2  register address, hex. For the "P1"/"P2"/"BP1" row types this is
 *         actually <command_code_hi><data_byte_lo> packed together, not a
 *         plain address - see the row-type dispatch below.
 *   col3  register address, decimal (redundant, ignored)
 *   col4  register name ("CRC_USER_Multi_N"/"TRIM*" rows are reference
 *         values or non-writes, not configuration writes)
 *   col5  value, hex
 *   col6  value, decimal (redundant, ignored)
 *   col7  row type: "1"/"2" = byte/word write; "Bn" = block write of n
 *         bytes (n<=4: the whole value is already in this one row; n>4:
 *         one byte per row, n consecutive rows, MSByte first, concatenated
 *         into a single block write); "P1"/"P2"/"BP1" = process-call style
 *         write-word to a command code encoded in col2's high byte
 *
 * Rows before "END" are real writes. CRC_CHECK_START/STOP brackets the
 * per-group reference CRC (read back from B8h after STORE_USER, compared
 * here as a log-only sanity check - not currently fatal, since the exact
 * B8h framing for MP29526 specifically is still unconfirmed).
 * CUSTOM_START/STOP and anything after CRC_CHECK_STOP is readback
 * verification data, not writes, and is ignored entirely.
 *
 * NOT implemented here (deferred - registers/values not confirmed for this
 * part, and guessing wrong risks writing to unrelated live registers):
 *   - VR14.cloud protection check and its page 0Bh unlock sequence
 *   - E0h password unlock (E0h is only read, informationally, at start)
 *   - Configuration ID (EFh) / REGMAP_REV (EDh) pre-checks against the chip
 * ========================================================================== */
...
/* col1: "<group><page>" (3 digits) or "<page>" (1-2 digits) - see the
 * column-layout comment above. Not a hex bitmask split. */
...
/*
 * STORE_USER + wait + B8h CRC readback (log-only compare against the file's
 * CRC_CHECK block, if captured) + RESTORE_USER + wait + STATUS_CML check,
 * for whichever group's registers were just written (0 = the shared/user
 * registers written before the first multi-config group starts, folded
 * into that first group's store per AN255's flow).
 */
...
static bool mp29526_stream_switch_group(uint8_t new_group)
{
	...
	/*
	 * Group 0 (shared/user registers) gets no store of its own - AN255's
	 * flow writes them, then selects group 1 and writes its registers,
	 * and only then does the first STORE_USER, committing the shared
	 * registers together with group 1. So the store here only fires
	 * when leaving an actual multi-config group (1-5), not group 0.
	 */
...
static bool mp29526_stream_flush_block(void)
{
	...
	/* Wire order is the file's row order reversed - confirmed against a
	 * real GUI capture (IC_DEVICE_ID's 8 rows come out on the wire in the
	 * opposite order from how they're listed in the file). */
...
static bool mp29526_stream_apply_row(const struct mp29526_row *row)
{
	...
	if (!strcmp(row->tag, "1") || !strcmp(row->tag, "2")) {
		...
		/* checked after the P1/P2/BP1 branch above, since "BP1" also
		 * starts with 'B' and would otherwise be misread as a block
		 * tag with atoi("P1") == 0. */
		...
		if (n <= 4) {
			...
			uint8_t payload[5];
			payload[0] = n;
			/* wire order is the hex string's byte-pairs reversed - see
			 * mp29526_stream_flush_block()'s comment for the same rule
			 * applied to the n>4, multi-row case. */
			for (uint8_t i = 0; i < n; i++)
 ...
 * group; they're never written. Our exported file always shows col1="0"
 * for these and encodes the group in the REG_NAME suffix
 * ("CRC_USER_Multi_<N>") instead of the 3-digit col1 AN255's own example
 * uses, so try the name suffix first and fall back to col1's group digit. */
			     ...
			     bool is_first, bool is_last)
{
	...
	if (is_first) {
		...
		/*
		 * Informational only: AN255 gates programming on a password
		 * at E0h when "VR14.cloud" protection is enabled, but the
		 * actual password value and whether MP29526 uses the same
		 * E0h/page-0Bh scheme as the MP29426 family are both
		 * unconfirmed. Read-only, so safe either way; just logged
		 * for comparison against a real GUI capture.
		 */
		uint8_t pw[2] = { 0 };
		if (mp29526_i2c_read(bus, addr, 0xE0, pw, sizeof(pw)))
			LOG_INF("E0h readback: 0x%02x 0x%02x (informational, unconfirmed framing)",
				pw[0], pw[1]);
